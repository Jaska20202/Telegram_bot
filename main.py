...
async def konspekt_qildim_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Yozgan qo'llaringiz dard ko'rmasin! Endi konspektingizni rasmga olib joylashtiring."
    )
...
def handle_response(text: str) -> str:
    ...
    if ("yaxshimiz?" in processed or
        ...
        return "Alhamdullilah, yaxshimiz. O'zingiz omonmisiz?"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- NEW CLEAN FALLBACK ECHO ---
    if response is None:
        response = f"Siz yozdingiz: {text}"
